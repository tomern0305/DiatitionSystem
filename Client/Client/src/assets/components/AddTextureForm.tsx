...
interface AddTextureFormProps {
  onAdd: (name: string) => Promise<void>;
}

const AddTextureForm: React.FC<AddTextureFormProps> = ({ onAdd }) => {
  const [newTextureName, setNewTextureName] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTextureName.trim() || isSubmitting) return;

    setIsSubmitting(true);
    try {
      await onAdd(newTextureName);
      setNewTextureName("");
    } catch (err) {
      console.error(err);
    } finally {
    ...
    >
      <div className="flex-1 w-full">
        ...
        <input
          ...
          placeholder="לדוגמא: נוזל מעובה סמיך..."
          required
        />
      </div>
      <button
